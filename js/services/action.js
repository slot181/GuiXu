    ...
    const ActionService = {
        ...
        normalizeMvuState(mvu) {
            try {
                ...
                // 工具函数
                const isMeta = (x) => x === '$__META_EXTENSIBLE__$' || x === '...';
                ...
                const flattenDeep = (arr) => {
                    let work = Array.isArray(arr) ? arr.slice() : [];
                    // 仅在出现嵌套数组时逐层拍平，直到不再嵌套
                    let guard = 0;
                    while (work.some(Array.isArray) && guard++ < 5) {
                        work = flattenOnce(work);
                    }
                    return work;
                };
                const extractLooseName = (s) => {
                    const text = String(s || '');
                    // 兼容松散文本/类YAML：匹配 名称|name|title|灵根名称
                    const m = text.match(/(?:^|[\r\n])\s*(?:名称|name|title|灵根名称)\s*[:：]\s*([^\r\n]+)/i);
                    return (m ? m[1] : text).trim();
                };
                const getId = (o) => o?.id ?? o?.ID ?? o?.Id ?? o?.uid ?? o?.UID ?? o?.Uid;
                ...
                const makeKey = (item) => {
                    ...
                    if (typeof item === 'string') {
                        ...
                        try {
                            ...
                            const loose = extractLooseName(item).toLowerCase();
                            return `name:${loose}`;
                        }
                    }
                    if (typeof item === 'object') {
                        ...
                        const name =
                            (item.name ||
                             ...
                             item.title ||
                             item['灵根名称'] ||
                             (item.data && (item.data.name || item.data['名称'] || item.data.title)) ||
                             ''
                            ).toString().trim().toLowerCase();
                        if (name) return `name:${name}`;

                        // 极端兜底：对无法识别的结构使用结构摘要作为键，避免完全失效
                        try { return `json:${JSON.stringify(item)}`; } catch { return 'object'; }
                    }
                    return String(item);
                };
                ...
                listKeys.forEach(k => {
                    ...
                    let work = flattenDeep(arr).filter(v => !isMeta(v) && v != null);
                    ...
                    // 针对人物关系列表：同步清理其内部的 event_history / 物品列表 的嵌套与重复
                    if (k === '人物关系列表') {
                        for (let i = 0; i < sd[k][0].length; i++) {
                            const relRaw = sd[k][0][i];
                            if (!relRaw || typeof relRaw === 'string') {
                                // 字符串化条目不作强行解析，避免破坏原始形态
                                continue;
                            }
                            try {
                                const relObj = relRaw;
                                ...
                                // 清理过往事件
                                if (Array.isArray(relObj.event_history)) {
                                    const eh = flattenDeep(relObj.event_history).filter(x => x && !isMeta(x));
                                    const seenEh = new Set();
                                    const outEh = [];
                                    for (const e of eh) {
                                        const keyEh = String(e);
                                        if (seenEh.has(keyEh)) continue;
                                        seenEh.add(keyEh);
                                        outEh.push(e);
                                    }
                                    relObj.event_history = outEh;
                                }

                                // 清理物品列表
                                if (Array.isArray(relObj.物品列表)) {
                                    const items = flattenDeep(relObj.物品列表).filter(x => x && !isMeta(x));
                                    const seenItem = new Set();
                                    const outItems = [];
                                    for (const itRaw of items) {
                                        const keyIt = makeKey(itRaw);
                                        if (seenItem.has(keyIt)) continue;
                                        seenItem.add(keyIt);
                                        outItems.push(itRaw);
                                    }
                                    relObj.物品列表 = outItems;
                                }

                                sd[k][0][i] = relObj;
                            } catch (_) { /* ignore 单条失败不影响整体 */ }
                        }
                    }
                });

                // === 追加的深度去重/规范化（不改变语义，仅做反重复与结构修复） ===
                // 1) 工具：special_effects 去重（保留占位符的同时仅保留一个）
                const normalizeEffectsArray = (eff) => {
                    try {
                        // 统一成数组
                        let arr = eff;
                        ...
                        if (!Array.isArray(arr)) return eff;
                        const out = [];
                        const seen = new Set();
                        let metaSeen = false;
                        for (const v of flattenDeep(arr)) {
                            if (isMeta(v)) { if (!metaSeen) { out.push(v); metaSeen = true; } continue; }
                            if (v == null) continue;
                            const key = typeof v === 'string' ? v.trim() : (function(){ try { return JSON.stringify(v); } catch { return String(v); }})();
                            if (!key) continue;
                            if (seen.has(key)) continue;
                            seen.add(key);
                            out.push(v);
                ...
                // 2) 工具：KV 容器去重（attributes_bonus / 百分比加成）
                // - 若已是对象：直接返回（对象本身不存在重复键）
                // - 若是数组/字符串：尽力解析为对象，重复键按“最后一次出现”生效
                const normalizeKV = (kv) => {
                    try {
                        if (!kv) return kv;
                        if (typeof kv === 'object' && !Array.isArray(kv)) return kv;
                        const out = {};
                        const acceptPair = (k, v) => {
                            if (k == null) return;
                            const key = String(k).trim();
                            if (!key) return;
                            // 值为字符串 '[object Object]' 时尝试忽略，或直接使用原值
                            out[key] = (v === '[object Object]' ? String(v) : v);
                        };
                        if (Array.isArray(kv)) {
                            for (const it of flattenDeep(kv)) {
                                if (!it || isMeta(it)) continue;
                                if (Array.isArray(it) && it.length >= 2) {
                                    acceptPair(it[0], it[1]);
                                } else if (typeof it === 'object') {
                                    for (const [k, v] of Object.entries(it)) acceptPair(k, v);
                                } else if (typeof it === 'string') {
                                    const m = it.match(/^([^:：]+)\s*[:：]\s*(.+)$/);
                                    if (m) acceptPair(m[1], m[2]);
                                }
                            }
                            return out;
                        }
                ...
                // 3) 工具：规范化一个“可带词条与加成”的条目
                const normalizeOneItem = (item) => {
                    try {
                        if (!item) return item;
                        let obj = item;
                        if (typeof obj === 'string') {
                            try { obj = JSON.parse(obj); } catch { return item; }
                        }
                        if (obj && typeof obj === 'object') {
                            if (obj.special_effects != null) obj.special_effects = normalizeEffectsArray(obj.special_effects);
                            if (obj['词条效果'] != null) obj['词条效果'] = normalizeEffectsArray(obj['词条效果']);
                            if (obj['词条'] != null) obj['词条'] = normalizeEffectsArray(obj['词条']);
                            if (obj.attributes_bonus != null) obj.attributes_bonus = normalizeKV(obj.attributes_bonus);
                            if (obj['属性加成'] != null) obj['属性加成'] = normalizeKV(obj['属性加成']);
                            if (obj['百分比加成'] != null) obj['百分比加成'] = normalizeKV(obj['百分比加成']);
                            if (obj.percent_bonus != null) obj.percent_bonus = normalizeKV(obj.percent_bonus);
                        }
                        // 保持原始类型（字符串化则写回字符串）
                        if (typeof item === 'string') {
                            try { return JSON.stringify(obj); } catch { return item; }
                        }
                        return obj;
                    } catch { return item; }
                };

                // 4) 装备槽（主修/辅修/武器/防具/饰品/法宝1）内部的 special_effects/加成 去重
                (['主修功法','辅修心法','武器','防具','饰品','法宝栏1']).forEach(slotKey => {
                    const w = sd[slotKey];
                    if (!w) return;
                    // 槽位通常是 [ item ] 或 []
                    if (Array.isArray(w) && w.length > 0) {
                        if (w[0] && (typeof w[0] === 'object' || typeof w[0] === 'string')) {
                            w[0] = normalizeOneItem(w[0]);
                        }
                ...
                perItemNormalizeTargets.forEach(k => {
                    const arrWrapper = sd[k];
                    if (!arrWrapper || !Array.isArray(arrWrapper) || arrWrapper.length === 0) return;
                    const arr = arrWrapper[0];
                    if (!Array.isArray(arr)) return;
                    sd[k][0] = arr.map(it => normalizeOneItem(it));
                });

                // 6) 人物关系列表内：inherent_abilities（灵根/天赋）与 当前状态 去重
                if (sd['人物关系列表'] && Array.isArray(sd['人物关系列表'][0])) {
                    const rels = sd['人物关系列表'][0];
                    for (let i = 0; i < rels.length; i++) {
                        const relRaw = rels[i];
                        if (!relRaw || typeof relRaw === 'string') continue; // 保持字符串原貌
                        const relObj = relRaw;

                        // inherent_abilities 可能是对象/数组/字符串化对象
                        let inh = relObj['inherent_abilities'] ?? relObj['内在能力'];
                        if (typeof inh === 'string') {
                            try { inh = JSON.parse(inh); } catch { inh = inh; }
                        }
                        if (Array.isArray(inh) && inh.length === 1) {
                            if (typeof inh[0] === 'string') { try { inh = JSON.parse(inh[0]); } catch {} }
                            else if (inh[0] && typeof inh[0] === 'object') inh = inh[0];
                        }
                        if (inh && typeof inh === 'object') {
                            // 灵根
                            if (inh['灵根']) inh['灵根'] = normalizeOneItem(inh['灵根']);
                            // 天赋：数组去重 + 每个元素内部去重
                            let tf = inh['天赋'];
                            if (typeof tf === 'string') { try { tf = JSON.parse(tf); } catch { tf = tf; } }
                            if (Array.isArray(tf)) {
                                // 去重（按 id/name）
                                const seen = new Set();
                                const out = [];
                                for (const t of flattenDeep(tf)) {
                                    if (!t || isMeta(t)) continue;
                                    const key = makeKey(t);
                                    if (seen.has(key)) continue;
                                    seen.add(key);
                                    out.push(normalizeOneItem(t));
                                }
                                inh['天赋'] = out;
                            }
                            // 写回
                            if (relObj['inherent_abilities'] != null) relObj['inherent_abilities'] = inh;
                            if (relObj['内在能力'] != null) relObj['内在能力'] = inh;
                        }

                        // 关系对象内“当前状态”字段：拍平 + 去重（按 id/name）
                        let relStatuses = relObj['当前状态'] ?? relObj['状态'];
                        // 兼容多种外观：字符串/嵌套数组/对象
                        try {
                            if (typeof relStatuses === 'string') {
                                const s = relStatuses.trim();
                                if ((s.startsWith('[') && s.endsWith(']')) || (s.startsWith('{') && s.endsWith('}'))) {
                                    try { relStatuses = JSON.parse(s); } catch {}
                                } else {
                                    relStatuses = [s];
                                }
                            }
                            if (Array.isArray(relStatuses) && relStatuses.length === 1 && Array.isArray(relStatuses[0])) {
                                relStatuses = relStatuses[0];
                            }
                            if (relStatuses && !Array.isArray(relStatuses) && typeof relStatuses === 'object') {
                                relStatuses = [relStatuses];
                            }
                            if (Array.isArray(relStatuses)) {
                                const seenS = new Set();
                                const outS = [];
                                for (const s of flattenDeep(relStatuses)) {
                                    if (!s || isMeta(s)) continue;
                                    const key = makeKey(s);
